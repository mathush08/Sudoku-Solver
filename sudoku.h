...
// time: O(1) since we are given a fixed number for how much the
//       loops will run, which is 9 times for all 3 aspects
// effects:  mutates *s
...
// time: O(1) since we are given a fixed number for how much the
//       loops will run, goes through entire board, which is 80
//       iterations. Also O(1)*O(1) would be still O(1)
